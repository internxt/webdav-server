...
module.exports = (callback, options) => {
  ...
  function callCallback() {
    --nb;
    if (nb <= 0) callback(successes, errors);
  }
  ...
  fs.readdir(root, (e, files) => {
    ...
    files.forEach((f1) => {
      ...
      fs.readdir(f1x, (e, files) => {
        ...
        files = files.filter(
          (f) => f.indexOf('.js') === f.length - 3 && f.indexOf('.') !== 0
        );
        nb += files.length;
        files.forEach((f) => {
          ...
          const info = {
            ...
            easyError: (e, cb) => (e, arg1, arg2, arg3) => {
              if (e) info.isValid(false, e);
              else cb(arg1, arg2, arg3);
            },
            ...
            req: (config, _codeStatusExpected, _callback) => {
              ...
              request(config, (e, res, body) => {
                if (e) return info.isValid(false, 'HTTP error.', e);
                ...
                if (
                  codeStatusExpected !== -1 &&
                  res.statusCode != codeStatusExpected
                )
                  return info.isValid(
                    false,
                    'Expected ' +
              ...
              request(config, (e, res, body) => {
                ...
                if (body) {
                  try {
                    ...
                    return info.isValid(
                      false,
                      'Invlid XML in the response body.',
                      body.toString()
                    );
                  }
                }
            ...
            expect: (nb) => {
              let callback = (valid, details) => {
                ...
                const prefix =
                  ...
                  (info.name ? ' : ' + info.name : '') +
                  ']';
                if (valid) success(prefix + details);
                else error(prefix + details);
                callCallback();
              };
              ...
              info.isValid = function (good, msg, error) {
                if (error) {
                  msg += ' :: ' + error;
                  if (options.showExceptions) console.error(error);
                }

                --nb;
                if (msg && allGood && !good) allMsg = msg;
          ...
          try {
            ...
            process.nextTick(() =>
              require(fx).default(info, (good, msg, e) =>
                info.isValid(good, msg, e)
              )
            );
          } catch (ex) {
            if (options.showExceptions) console.error(ex);
