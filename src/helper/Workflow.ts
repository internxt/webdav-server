...
export class Workflow {
  ...
  protected _done(subject: any, e?: any, data?: any) {
    ...
    if (e) {
      if (this.exitOnError) this.counter = -1000;
      if (this.errorFn) process.nextTick(() => this.errorFn(e));
      if (this.exitOnError) {
        this.started = false;
        return;
      }
    ...
    if (this.intermediateFn) this.intermediateFn(subject, e, data);
  ...
  each<T>(
    subjects: T[],
    fn: (subject: T, done: (error?: any, data?: any) => void) => void
  ) {
    this.counter = subjects.length;
    subjects.forEach((s) =>
      process.nextTick(() => fn(s, (e, d) => this._done(s, e, d)))
    );
    return this;
  }
    ...
    process.nextTick(() => {
      ...
      for (const name in object)
        process.nextTick(() => {
          ...
          fn(name, object[name], (e, d) =>
            this._done({ [name]: object[name] }, e, d)
          );
        });
    });
    return this;
  ...
  first(fn: (data?: any) => void, notFound: () => void) {
    ...
    if (this.counter === 0) process.nextTick(() => this.notFound());
    ...
    if (this.counter === 0) process.nextTick(() => this.doneFn(this.data));
