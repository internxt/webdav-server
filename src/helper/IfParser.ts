...
import { Path, Resource } from '../manager';
...
function parseInternal(group: string) {
  const rex = /((not)|\[([^\]]+)\]|<(DAV:no-lock)>|<([^>]+)>|([^\s]+))/gi;
  ...
  if (andArray.length) return (r, callback) => callback(null, true);
...
export function parseIfHeader(ifHeader: string) {
  ...
  while (match) {
    if (match[1]) oldPath = url.parse(match[1]).path;
  ...
  return function (
    ...
    orArray.forEach((a) => {
      if (!a.path) a.actions(resource, done);
      else {
