...
export class SimpleUserManager
  implements ITestableUserManager, IListUserManager
{
  protected users: any;
  ...
  getUserByName(name: string, callback: (error: Error, user?: IUser) => void) {
    if (!this.users[name]) callback(Errors.UserNotFound);
    else callback(null, this.users[name]);
  }
  ...
  getUsers(callback: (error: Error, users: IUser[]) => void) {
    ...
    for (const name in this.users) users.push(this.users[name]);
