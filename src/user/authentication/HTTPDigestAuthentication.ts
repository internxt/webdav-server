...
export class HTTPDigestAuthentication implements HTTPAuthentication {
  constructor(
    public userManager: IListUserManager,
    public realm: string = 'realm',
    public nonceSize: number = 50
  ) {}
  ...
  getUser(
    ...
    if (!authProps.algorithm) authProps.algorithm = 'MD5';

    this.userManager.getUserByName(authProps.username, (e, user) => {
      if (e) return onError(Errors.BadAuthentication);

      let ha1 = md5(
        `${authProps.username}:${this.realm}:${
      ...
      if (digestUri !== requestedUri) {
        ...
        switch (digestUri.length - requestedUri.length) {
          case -1:
            uriMismatch =
              !startsWith(requestedUri, digestUri) ||
              requestedUri[digestUri.length] !== '/';
            break;

          case 1:
            uriMismatch =
              !startsWith(digestUri, requestedUri) ||
              digestUri[requestedUri.length] !== '/';
            break;
      ...
      if (authProps.qop === 'auth-int')
        return onError(Errors.WrongHeaderFormat);
      // ha2 = md5(ctx.request.method.toString().toUpperCase() + ':' + digestUri + ':' + md5(...));
      else
        ha2 = md5(
          `${ctx.request.method.toString().toUpperCase()}:${digestUri}`
        );

      let result;
      if (authProps.qop === 'auth-int' || authProps.qop === 'auth')
        result = md5(
          `${ha1}:${authProps.nonce}:${authProps.nc}:${authProps.cnonce}:${authProps.qop}:${ha2}`
        );
      else result = md5(`${ha1}:${authProps.nonce}:${ha2}`);

      if (result.toLowerCase() === authProps.response.toLowerCase())
        callback(Errors.None, user);
      else onError(Errors.BadAuthentication);
    });
  }
}
