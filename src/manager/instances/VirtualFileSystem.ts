...
export class VirtualFileSystemResource {
  ...
  constructor(data: VirtualFileSystemResource | ResourceType) {
    ...
    if (
      data &&
      (data as ResourceType).isFile !== undefined &&
      (data as ResourceType).isDirectory !== undefined
    ) {
      rs = {
...
export class VirtualFileReadable extends Readable {
  ...
  _read(size: number) {
    while (true) {
      ...
      if (!this.push(this.contents[this.blockIndex])) break;
    }
  }
}
...
export const VirtualSerializerVersions = {
  ...
  instances: [new VirtualSerializer()] as FileSystemSerializer[],
};
...
export class VirtualFileSystem extends FileSystem {
  ...
  protected _delete(
    ...
    for (const path in this.resources) {
      if (startsWith(path, sPath)) delete this.resources[path];
    }
  ...
  protected _openWriteStream(
    ...
    if (resource === undefined) return callback(Errors.ResourceNotFound);
  ...
  protected getPropertyFromResource(
    ...
    const resource = this.resources[path.toString()];
    if (!resource) return callback(Errors.ResourceNotFound);
