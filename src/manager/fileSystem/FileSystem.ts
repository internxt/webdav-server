...
import { BasicPrivilege, PrivilegeManager } from '../../user';
import { FileSystemEvent, WebDAVServer, RequestContext } from '../../server';
...
import { LockScope, LockType, LockKind } from '../../resource';
...
import { ensureValue, promisifyCall } from '../../helper/promise';
...
class BufferedIsLocked {
  ...
  isLocked(callback: ReturnCallback<boolean>) {
    if (this._isLocked !== null) return callback(null, this._isLocked);
...
export abstract class FileSystem implements ISerializableFileSystem {
  ...
  fastExistCheckEx(
    ...
    if (!this._fastExistCheck) return callback();

    const path = new Path(_path);
    this._fastExistCheck(ctx, path, (exists) => {
      if (!exists) errorCallback(Errors.ResourceNotFound);
      else callback();
    });
  }
  ...
  fastExistCheckExReverse(
    ...
    const path = new Path(_path);
    this._fastExistCheck(ctx, path, (exists) => {
      if (exists) errorCallback(Errors.ResourceAlreadyExists);
      else callback();
    });
  }
   ...
   */
  protected fastExistCheck(
    ctx: RequestContext,
    _path: Path | string,
    callback: (exists: boolean) => void
  ): void {
    if (!this._fastExistCheck) return callback(true);
  ...
  createAsync(
    ...
    return promisifyCall((cb) =>
      this.create(ctx, path, type, createIntermediates, cb)
    );
  }
  ...
  create(
    ...
    _path: Path | string,
    type: ResourceType,
    _createIntermediates: boolean | SimpleCallback,
    _callback?: SimpleCallback
  ): void {
    const createIntermediates = ensureValue(
      _callback ? (_createIntermediates as boolean) : undefined,
      false
    );
    const callbackFinal = _callback
      ? _callback
      : (_createIntermediates as SimpleCallback);
    const path = new Path(_path);

    const callback: SimpleCallback = (e) => {
      if (!e) this.emit('create', ctx, path, { type, createIntermediates });
      callbackFinal(e);
    };

    if (!this._create) return callback(Errors.InvalidOperation);

    this.emit('before-create', ctx, path, { type, createIntermediates });

    issuePrivilegeCheck(this, ctx, path, 'canWrite', callback, () => {
      const go = () => {
        ctx.server.options.storageManager.evaluateCreate(
          ctx,
          this,
          path,
          type,
          (size) => {
            ctx.server.options.storageManager.reserve(
              ctx,
              this,
              size,
              (reserved) => {
                if (!reserved) return callback(Errors.InsufficientStorage);
      ...
      this.isLocked(ctx, path, (e, locked) => {
        if (e || locked) return callback(locked ? Errors.Locked : e);
        ...
        if (!this._etag)
          return this.lastModifiedDate(ctx, path, (e, date) => {
            ...
            callback(
              null,
              '"' +
                crypto.createHash('md5').update(date.toString()).digest('hex') +
                '"'
            );
          });
  ...
  delete(
    ...
    const callback: SimpleCallback = (e) => {
      if (!e) this.emit('delete', ctx, path, { depth });
      callbackFinal(e);
    };

    if (!this._delete) return callback(Errors.InvalidOperation);

    this.emit('before-delete', ctx, path, { depth });

    issuePrivilegeCheck(this, ctx, path, 'canWrite', callback, () => {
      this.isLocked(ctx, path, (e, isLocked) => {
        if (e || isLocked) return callback(e ? e : Errors.Locked);
              ...
              (e) => {
                if (!e) {
                  this.type(ctx, path, (e, type) => {
                    ctx.server.options.storageManager.evaluateContent(
                      ctx,
                      this,
                      contentSize,
                      (reservedContentSize) => {
  ...
  openWriteStreamAsync(
    ...
    return promisifyCall((cb) =>
      this.openWriteStream(
        ...
        estimatedSize,
        (e, data1, data2) =>
          cb(e, e ? undefined : { stream: data1, created: data2 })
      )
    );
  }
  ...
  openWriteStream(
    ctx: RequestContext,
    path: Path | string,
    callback: Return2Callback<Writable, boolean>
  ): void;
  /**
   * Open a stream to write the content of the resource.
   *
  ...
  openWriteStream(
    ...
    _mode:
      ...
      | boolean
      | number
      | Return2Callback<Writable, boolean>,
    _targetSource?: boolean | number | Return2Callback<Writable, boolean>,
    _estimatedSize?: number | Return2Callback<Writable, boolean>,
    _callback?: Return2Callback<Writable, boolean>
  ): void {
    let targetSource = false;
    for (const obj of [_mode, _targetSource])
      if (obj && obj.constructor === Boolean) targetSource = obj as boolean;

    let estimatedSize = -1;
    for (const obj of [_mode, _targetSource, _estimatedSize])
      if (obj && obj.constructor === Number) estimatedSize = obj as number;
    ...
    const mode =
      _mode && _mode.constructor === String
        ? (_mode as OpenWriteStreamMode)
        : 'mustExist';
    const path = new Path(_path);
    let created = false;

    const callback: Return2Callback<Writable, boolean> = (
      e,
      stream,
      created
    ) => {
      if (!e)
        this.emit('openWriteStream', ctx, path, {
          targetSource,
          mode,
          estimatedSize,
          created,
    ...
    if (!this._openWriteStream) return callback(Errors.InvalidOperation);

    this.emit('before-openWriteStream', ctx, path, {
      targetSource,
      mode,
      estimatedSize,
      created,
    ...
    issuePrivilegeCheck(
      ...
      ctx,
      path,
      targetSource ? 'canWriteContentSource' : 'canWriteContentTranslated',
      callback,
      () => {
        this.isLocked(ctx, path, (e, isLocked) => {
          if (e || isLocked) return callback(e ? e : Errors.Locked);
              ...
              ctx.server.options.storageManager.evaluateContent(
                ...
                (sizeStored) => {
                  if (
                    ...
                    estimatedSize === null ||
                    (estimatedSize.constructor === Number && estimatedSize <= 0)
                  ) {
                    ctx.server.options.storageManager.available(
                      ctx,
                      this,
                      (available) => {
                        if (available === -1) return finalGo(callback);
                        if (available === 0)
                          return callback(Errors.InsufficientStorage);

                        let nb = 0;
                        finalGo((e, wStream, created) => {
                          if (e) return callback(e, wStream, created);
                    ...
                    ctx.server.options.storageManager.evaluateContent(
                      ...
                      (estimatedSizeStored) => {
                        ctx.server.options.storageManager.reserve(
                          ctx,
                          this,
                          estimatedSizeStored - sizeStored,
                          (reserved) => {
                            if (!reserved)
                              return callback(Errors.InsufficientStorage);
                            finalGo(callback);
  ...
  openReadStreamAsync(
    ...
    return promisifyCall((cb) =>
      this.openReadStream(ctx, path, targetSource, estimatedSize, cb)
    );
  }
  ...
  openReadStream(
    ctx: RequestContext,
    _path: Path | string,
    _targetSource: boolean | number | ReturnCallback<Readable>,
    _estimatedSize?: number | ReturnCallback<Readable>,
    _callback?: ReturnCallback<Readable>
  ): void {
    const targetSource = ensureValue(
      _targetSource.constructor === Boolean
        ? (_targetSource as boolean)
        : undefined,
      false
    );
    const estimatedSize = ensureValue(
      _callback
        ? (_estimatedSize as number)
        : _estimatedSize
        ? (_targetSource as number)
        : undefined,
      -1
    );
    const callbackFinal = _callback
      ? _callback
      : _estimatedSize
      ? (_estimatedSize as ReturnCallback<Readable>)
      : (_targetSource as ReturnCallback<Readable>);
    const path = new Path(_path);

    const callback: ReturnCallback<Readable> = (e, stream) => {
      if (!e)
        this.emit('openReadStream', ctx, path, {
          targetSource,
          estimatedSize,
          stream,
        });
    ...
    this.emit('before-openReadStream', ctx, path, {
      targetSource,
      estimatedSize,
    });
    ...
    issuePrivilegeCheck(
      ...
      ctx,
      path,
      targetSource ? 'canReadContentSource' : 'canReadContentTranslated',
      callback,
      () => {
        this.fastExistCheckEx(ctx, path, callback, () => {
          if (!this._openReadStream) return callback(Errors.InvalidOperation);
  ...
  moveAsync(
    ...
    return promisifyCall((cb) =>
      this.move(ctx, pathFrom, pathTo, overwrite, cb)
    );
  }
  ...
  move(
    ctx: RequestContext,
    _pathFrom: Path | string,
    _pathTo: Path | string,
    _overwrite: boolean | ReturnCallback<boolean>,
    _callback?: ReturnCallback<boolean>
  ): void {
    const callbackFinal = _callback
      ? _callback
      : (_overwrite as ReturnCallback<boolean>);
    const overwrite = ensureValue(
      _callback ? (_overwrite as boolean) : undefined,
      false
    );
  ...
  copyAsync(
    ...
    return promisifyCall((cb) =>
      this.copy(ctx, pathFrom, pathTo, overwrite, depth)
    );
  }
  ...
  copy(
    ...
    _pathFrom: Path | string,
    _pathTo: Path | string,
    _overwrite: boolean | number | ReturnCallback<boolean>,
    _depth?: number | ReturnCallback<boolean>,
    _callback?: ReturnCallback<boolean>
  ): void {
    const overwrite = ensureValue(
      _overwrite.constructor === Boolean ? (_overwrite as boolean) : undefined,
      false
    );
    const depth = ensureValue(
      _callback
        ? (_depth as number)
        : !_depth
        ? -1
        : _overwrite.constructor === Number
        ? (_overwrite as number)
        : undefined,
      -1
    );
    const callbackFinal = _callback
      ? _callback
      : _depth
      ? (_depth as ReturnCallback<boolean>)
      : (_overwrite as ReturnCallback<boolean>);
    const pathFrom = new Path(_pathFrom);
    const pathTo = new Path(_pathTo);
  ...
  renameAsync(
    ...
    return promisifyCall((cb) =>
      this.rename(ctx, pathFrom, newName, overwrite, cb)
    );
  }
  ...
  rename(
    ...
    const callback: ReturnCallback<boolean> = (e, overrided) => {
      if (!e) this.emit('rename', ctx, pathFrom, { newName, overrided });
      callbackFinal(e, overrided);
    };
        ...
        this.isLocked(ctx, pathFrom, (e, isLocked) => {
          ...
          if (pathFrom.isRoot()) {
            this.getFullPath(ctx, (e, fullPath) => {
              if (fullPath.isRoot()) return callback(Errors.InvalidOperation);
                  ...
                  ctx.server.getFileSystem(newPath, (fs, _, subPath) => {
                    const go = (overwritten: boolean) => {
                      ctx.server.setFileSystem(newPath, this, (successed) => {
                        ...
                        ctx.server.removeFileSystem(fullPath, () =>
            ...
            this.fastExistCheckEx(ctx, pathFrom, callback, () => {
              this.fastExistCheckExReverse(
                ctx,
                pathFrom.getParent().getChildPath(newName),
                callback,
                () => {
  ...
  mimeType(
    ctx: RequestContext,
    _path: Path | string,
    _targetSource: boolean | ReturnCallback<string>,
    _callback?: ReturnCallback<string>
  ): void {
    const targetSource = ensureValue(
      _callback ? (_targetSource as boolean) : undefined,
      true
    );
    const callback = _callback
      ? _callback
      : (_targetSource as ReturnCallback<string>);
    const path = new Path(_path);
      ...
      () => {
        this.fastExistCheckEx(ctx, path, callback, () => {
          if (this._mimeType) {
            ...
            StandardMethods.standardMimeType(
              ctx,
              this,
              path,
              targetSource,
  ...
  size(
    ctx: RequestContext,
    path: Path | string,
    _targetSource: boolean | ReturnCallback<number>,
    _callback?: ReturnCallback<number>
  ): void {
    const targetSource = ensureValue(
      _callback ? (_targetSource as boolean) : undefined,
      false
    ...
    const callback = _callback
      ? _callback
      : (_targetSource as ReturnCallback<number>);
    ...
    issuePrivilegeCheck(
      ...
      () => {
        this.fastExistCheckEx(ctx, pPath, callback, () => {
          if (!this._size) return callback(null, undefined);
  ...
  lockManager(
    ctx: RequestContext,
    path: Path | string,
    callback: ReturnCallback<ILockManagerAsync>
  ): void {
    const pPath = new Path(path);
        ...
        (e, lm) => {
          ...
          const manager = {
              ...
              issuePrivilegeCheck(
                ...
                () => {
                  buffIsLocked.isLocked((e, isLocked) => {
                    ...
                    lm.setLock(lock, (e) => {
                      if (!e) fs.emit('lock-set', ctx, pPath, { lock });
                      callback(e);
                    });
                  });
                }
            ...
            refreshAsync(uuid: string, timeoutSeconds: number): Promise<Lock> {
              return promisifyCall((cb) =>
                manager.refresh(uuid, timeoutSeconds, cb)
              );
            },
            refresh(
              uuid: string,
              timeoutSeconds: number,
              callback: ReturnCallback<Lock>
            ): void {
              fs.emit('before-lock-refresh', ctx, pPath, {
                uuid,
                ...
                () => {
                  buffIsLocked.isLocked((e, isLocked) => {
                    ...
                    lm.refresh(uuid, timeoutSeconds, (e, lock) => {
                      if (!e)
                        fs.emit('lock-refresh', ctx, pPath, {
                          uuid,
                          timeout: timeoutSeconds,
                          lock,
                        });
  ...
  protected abstract _lockManager(
    path: Path,
    ctx: LockManagerInfo,
        ...
        (e, pm) => {
          ...
          callback(null, {
            setProperty(
              ...
              fs.emit('before-property-set', ctx, pPath, {
                name,
                value,
                attributes,
              });
                ...
                () => {
                  buffIsLocked.isLocked((e, isLocked) => {
                    ...
                    pm.setProperty(name, value, attributes, (e) => {
                      if (!e)
                        fs.emit('property-set', ctx, pPath, {
                          name,
                          value,
                          attributes,
                        });
                ...
                () => {
                  buffIsLocked.isLocked((e, isLocked) => {
                    ...
                    pm.removeProperty(name, (e) => {
                      if (!e) fs.emit('property-remove', ctx, pPath, { name });
                      callback(e);
                    });
                  });
                }
                ...
                () => {
                  pm.getProperties((e, bag) => {
                    ...
                    ctx.server.options.storageManager.available(
                      ...
                      this,
                      (availableSize) => {
                        if (availableSize === -1) return callback(e, bag);

                        ctx.server.options.storageManager.reserved(
                          ctx,
                          this,
                          (reservedSize) => {
  ...
  protected abstract _propertyManager(
    path: Path,
  ...
  readDirAsync(
    ...
    return promisifyCall((cb) =>
      this.readDir(ctx, path, retrieveExternalFiles, cb)
    );
  }
  ...
  readDir(
    ctx: RequestContext,
    path: Path | string,
    _retrieveExternalFiles: boolean | ReturnCallback<string[]>,
    _callback?: ReturnCallback<string[]>
  ): void {
    const retrieveExternalFiles = ensureValue(
      _callback ? (_retrieveExternalFiles as boolean) : undefined,
      false
    );
    const __callback = _callback
      ? _callback
      : (_retrieveExternalFiles as ReturnCallback<string[]>);
    const pPath = new Path(path);
      ...
      this.getFullPath(ctx, (e, fsFullPath) => {
        new Workflow()
          ...
          .done(() =>
            __callback(
              null,
              data.filter((p) => !!p).map((p) => p.fileName())
            )
          );
      });
    };
    ...
    issuePrivilegeCheck(this, ctx, pPath, 'canReadProperties', callback, () => {
      this.fastExistCheckEx(ctx, pPath, callback, () => {
        const next = (base: Path[]) => {
          if (!this._readDir) return callback(null, base);
          ...
          this._readDir(
            ...
            (e, paths) => {
              ...
              if (paths.length === 0) return callback(null, base);

              if (paths[0].constructor === String)
                base = base.concat(
                  (paths as string[]).map((s) => pPath.getChildPath(s))
                );
              else base = base.concat(paths as Path[]);
        ...
        if (!retrieveExternalFiles) return next([]);

        this.getFullPath(ctx, (e, thisFullPath) => {
          if (e) return callback(e);

          ctx.server.getChildFileSystems(
            thisFullPath.getChildPath(pPath),
            (fss) => {
              this.localize(
                ctx,
                fss.map((f) => f.path),
  ...
  protected static neutralizeEmptyDate(
    date: number,
    defaultDate?: number
  ): number {
    if (!date || isNaN(date)) {
      if (defaultDate === undefined || defaultDate === null) defaultDate = 0;
      return defaultDate;
    } else {
      return date;
    }
  }
  protected static neutralizeEmptyDateCallback = (
    callback: ReturnCallback<number>
  ): ReturnCallback<number> => {
  ...
  webName(
    ...
    issuePrivilegeCheck(this, ctx, pPath, 'canReadProperties', callback, () => {
      this.fastExistCheckEx(ctx, pPath, callback, () => {
        if (pPath.isRoot())
          this.getFullPath(ctx, (e, pPath) =>
            callback(e, e ? null : pPath.fileName())
          );
        else callback(null, pPath.fileName());
      });
    });
  }
  ...
  displayName(
    ...
    issuePrivilegeCheck(this, ctx, pPath, 'canReadProperties', callback, () => {
      this.fastExistCheckEx(ctx, pPath, callback, () => {
        if (!this._displayName) return this.webName(ctx, pPath, callback);
  ...
  addSubTreeAsync(
    ctx: RequestContext,
    resourceType: ResourceType | string | Buffer
  ): Promise<void>;
  /**
  ...
  addSubTree(
    ...
    _rootPath:
      ...
      | string
      | Buffer,
    _tree: SubTree | ResourceType | SimpleCallback | string | Buffer,
    _callback?: SimpleCallback
  ): void {
    const _rootPathIsPath = Path.isPath(_rootPath);
    const tree = _rootPathIsPath
      ? (_tree as SubTree | ResourceType)
      : (_rootPath as SubTree | ResourceType);
    const rootPath = _rootPathIsPath
      ? new Path(_rootPath as Path | string)
      : new Path('/');
    ...
    if (tree.constructor === ResourceType) {
      ...
      new Workflow()
        .each(Object.keys(tree), (name, cb) => {
          ...
          if (
            value.constructor === ResourceType ||
            value.constructor === String ||
            value.constructor === Buffer
          ) {
            this.addSubTree(ctx, childPath, value as any, cb);
          } else {
  ...
  listDeepLocks(
    ctx: RequestContext,
    startPath: Path | string,
    callback: ReturnCallback<{ [path: string]: Lock[] }>
  );
  /**
   * Search for locks in the parents, starting at the 'startPath' path.
   *
  ...
  listDeepLocks(
    ctx: RequestContext,
    startPath: Path | string,
    _depth: number | ReturnCallback<{ [path: string]: Lock[] }>,
    _callback?: ReturnCallback<{ [path: string]: Lock[] }>
  ) {
    const depth = ensureValue(_callback ? (_depth as number) : undefined, 0);
    const callback = _callback
      ? _callback
      : (_depth as ReturnCallback<{ [path: string]: Lock[] }>);
  ...
  getFullPath(
    ctx: RequestContext,
    path: Path | string,
    callback: ReturnCallback<Path>
  ): void;
  getFullPath(
    ctx: RequestContext,
    _path: Path | string | ReturnCallback<Path>,
    _callback?: ReturnCallback<Path>
  ): void {
    const path =
      !_path || typeof _path === 'function'
        ? undefined
        : new Path(_path as Path | string);
  ...
  localizeAsync(
    ctx: RequestContext,
    fullPath: Path | string | (string | Path)[]
  ): Promise<Path[]> {
  ...
  checkPrivilegeAsync(
    ctx: RequestContext,
    path: Path | string,
    privileges: BasicPrivilege | BasicPrivilege[]
  ): Promise<boolean>;
  checkPrivilegeAsync(
    ctx: RequestContext,
    path: Path | string,
  ...
  checkPrivilegeAsync(
    ctx: RequestContext,
    path: Path | string,
    privileges: string | string[] | BasicPrivilege | BasicPrivilege[]
  ): Promise<boolean> {
    return promisifyCall((cb) =>
      this.checkPrivilege(ctx, path, privileges as any, cb)
    );
  }
  ...
  checkPrivilege(
    ...
    if (privileges.constructor === String) privileges = [privileges as string];
   ...
   * @param path Path of the resource.
   */
  privilegeManagerAsync(
    ctx: RequestContext,
    path: Path | string
  ): Promise<PrivilegeManager> {
  ...
  protected _privilegeManager?(
    path: Path,
    info: PrivilegeManagerInfo,
    callback: ReturnCallback<PrivilegeManager>
  );
  ...
  isLocked(
    ...
    if (ctx.user && ctx.user.isAdministrator) return callback(null, false);

    const pPath = new Path(path);

    const checkThis = () => {
      this._lockManager(pPath, { context: ctx }, (e, lm) => {
        if (e === Errors.ResourceNotFound) return callback(null, false);
        if (e) return callback(e);
        ...
        lm.getLocks((e, locks) => {
          ...
          if (!ctx.user) return callback(null, locks.length > 0);

          if (
            locks.some(
              (l) =>
                ctx.user.uid !== l.userUid &&
                l.lockKind.scope.isSame(LockScope.Exclusive)
            )
          )
          ...
          for (const lock of locks) {
            if (lock.lockKind.scope.isSame(LockScope.Shared)) {
              isShared = true;
              if (lock.userUid === ctx.user.uid) return callback(null, false);
            }
          }
  ...
  on(
    ...
    listener: (ctx: RequestContext, path: Path, data?: any) => void
  ): this;
  /**
   * Attach a listener to an event.
   *
  ...
  on(
    ctx: RequestContext | WebDAVServer,
    event: FileSystemEvent,
    listener: (ctx: RequestContext, path: Path, data?: any) => void
  ): this {
    const server = (ctx as any).events
      ? (ctx as WebDAVServer)
      : (ctx as RequestContext).server;
    server.on(event, (ctx, fs, path) => {
      if (fs === this) listener(ctx, path);
    });
    return this;
  }
...
function issuePrivilegeCheck(
  fs: FileSystem,
  ctx: RequestContext,
  path: Path | string,
  privilege: BasicPrivilege | BasicPrivilege[],
  badCallback: SimpleCallback,
  goodCallback: () => void
) {
  fs.checkPrivilege(ctx, path, privilege, (e, can) => {
    if (e) badCallback(e);
    else if (!can) badCallback(Errors.NotEnoughPrivilege);
    else goodCallback();
  });
}
