...
export abstract class StandardMethods {
  ...
  public static standardMove(
    ...
    const go = (fullPathFrom?: Path) => {
      StandardMethods.standardCopy(
        ...
        -1,
        (e, overwritten) => {
          if (e) return callback(e, overwritten);
  ...
  public static standardCopy(
    ...
    const overwrite =
      _overwrite.constructor === Boolean ? (_overwrite as boolean) : false;
    const depth = _callback
      ? (_depth as number)
      : !_depth
      ? -1
    ...
    const go = () => {
        ...
        fsFrom.readDir(ctx, subPathFrom, false, (e, files) => {
          ...
          new Workflow()
            .each(files, (file, cb) =>
              StandardMethods.standardCopy(
                ctx,
                subPathFrom.getChildPath(file),
                fsFrom,
                subPathTo.getChildPath(file),
                fsTo,
                overwrite,
                subDepth,
                cb
      ...
      fsFrom.type(ctx, subPathFrom, (e, type) => {
        ...
        const startCopy = () => {
          ...
          if (type.isDirectory && depth !== 0) fns.push(copyChildren);
          if (type.isFile) fns.push(copyContent);
  ...
  public static standardMimeType(
    ctx: RequestContext,
    fs: FileSystem,
    path: Path,
  ...
  public static standardMimeType(
    ...
    path: Path,
    targetSource: boolean,
    _defaultMimeType: boolean | string | ReturnCallback<string>,
    _useWebName?: boolean | ReturnCallback<string>,
    ...
    fs.type(ctx, path, (e, type) => {
      ...
      } else callback(Errors.NoMimeTypeForAFolder, null);
    });
  }
}
