...
export function serialize(
  fileSystems: UnserializedData,
  callback: ReturnCallback<SerializedData>
) {
  ...
  new Workflow()
    .each(Object.keys(fileSystems), (path, cb) => {
      ...
      if (!serializer) return cb(); // Skip serialization
...
export function unserialize(
  ...
  callback: ReturnCallback<UnserializedData>
) {
  ...
  new Workflow()
    .each(Object.keys(serializedData), (path, cb) => {
      ...
      if (!serializer) return cb(new SerializerNotFound(sd.serializer));
