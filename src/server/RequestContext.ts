...
export class RequestContextHeaders {
  ...
  constructor(protected headers: { [name: string]: string | string[] }) {
    this.isSource =
      this.find('source', 'F').toUpperCase() === 'T' ||
      this.find('translate', 'T').toUpperCase() === 'F';
    this.host = this.find('Host', 'localhost');

    const depth = this.find('Depth');
    try {
      if (depth.toLowerCase() === 'infinity') this.depth = -1;
      else this.depth = Math.max(-1, parseInt(depth, 10));
    } catch (_) {
    ...
    try {
      this.contentLength = Math.max(
        0,
        parseInt(this.find('Content-length', '0'), 10)
      );
    } catch (_) {
  ...
  findBestAccept(defaultType: string = 'xml'): string {
    ...
    for (const value of accepts) {
      for (const name in regex) if (regex[name].test(value)) return name;
    }
...
export class DefaultRequestContextExternalOptions
  implements RequestContextExternalOptions
{
  headers: { [name: string]: string } = {
    host: 'localhost',
  };
...
export class RequestContext {
  ...
  protected constructor(
    ...
    if (this.rootPath) {
      ...
      if (this.rootPath === '/') this.rootPath = undefined;
    }
  }
  ...
  getResource(
    _path: Path | string | ReturnCallback<Resource>,
    _callback?: ReturnCallback<Resource>
  ): void {
    const path = Path.isPath(_path)
      ? new Path(_path as Path | string)
      : this.requested.path;
    const callback = _callback
      ? _callback
      : (_path as ReturnCallback<Resource>);
  ...
  fullUri(uri: string = null): string {
    if (!uri) uri = this.requested.uri;

    if (this.server.options.respondWithPaths)
      return this.rootPath ? this.rootPath + uri : uri;
    else return (this.prefixUri() + uri).replace(/([^:])\/\//g, '$1/');
  }

  prefixUri(): string {
    return (
      'http://' +
      this.headers.host.replace('/', '') +
...
export class ExternalRequestContext extends RequestContext {
  ...
  static create(
    ...
    const options =
      _options && _options.constructor !== Function
        ? (_options as RequestContextExternalOptions)
        : defaultValues;
    const callback = _callback
      ? _callback
      : _options && _options.constructor === Function
      ? (_options as (error: Error, ctx: ExternalRequestContext) => void)
      : () => {};

    if (defaultValues !== options) {
      for (const name in defaultValues)
        if (options[name] === undefined) options[name] = defaultValues[name];
    }

    const ctx = new ExternalRequestContext(
      server,
      options.url,
      options.headers
    );
...
export class HTTPRequestContext extends RequestContext {
  ...
  static create(
    ...
    request: http.IncomingMessage,
    response: http.ServerResponse,
    callback: (error: Error, ctx: HTTPRequestContext) => void
  ): void;
  static create(
    server: WebDAVServer,
    request: http.IncomingMessage,
  ...
  static create(
    ...
    request: http.IncomingMessage,
    response: http.ServerResponse,
    _rootPath: string | ((error: Error, ctx: HTTPRequestContext) => void),
    _callback?: (error: Error, ctx: HTTPRequestContext) => void
  ): void {
    const rootPath = _callback ? (_rootPath as string) : undefined;
    const callback = _callback
      ? _callback
      : (_rootPath as (error: Error, ctx: HTTPRequestContext) => void);
    ...
    response.setHeader(
      'Server',
      server.options.serverName + '/' + server.options.version
    );
    ...
    ctx.askForAuthentication(false, (e) => {
      ...
      server.httpAuthentication.getUser(ctx, (e, user) => {
        ...
        if (e && e !== Errors.UserNotFound) {
          if (
            server.options.requireAuthentification ||
            e !== Errors.MissingAuthorisationHeader
          )
  ...
  askForAuthentication(
    ...
    for (const name in auth) this.response.setHeader(name, auth[name]);
    callback(null);
  }
