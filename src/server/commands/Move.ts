...
import { Path, ResourceType, StandardMethods } from '../../manager/';
    ...
    ctx.getResource((e, r) => {
      ctx.checkIfHeader(r, () => {
        ...
        const cb = (e?: Error, overwritten?: boolean) => {
          ...
          } else if (overwritten) ctx.setCode(HTTPCodes.NoContent);
          else ctx.setCode(HTTPCodes.Created);
          callback();
        };

        ctx.server.getFileSystem(
          destination,
          (destFs, destRootPath, destSubPath) => {
            if (destFs !== r.fs) {
              // Standard method
              if (methodName === 'move')
