      ...
      ctx.checkIfHeader(r, () => {
        ...
        try {
            ...
            finalize = function () {
              const next = () => {
                ...
                codes.forEach((code) => {
                  ...
                  notifications[code].forEach((name) =>
                    prop.add(new XMLElementBuilder(name))
                  );
                  propstat
                    .ele('D:status')
                    .add('HTTP/1.1 ' + code + ' ' + STATUS_CODES[code]);
                });
                ...
                new Workflow()
                  ...
                  .done(() => {
                    if (notifications[HTTPCodes.OK]) {
                      notifications[HTTPCodes.FailedDependency] =
                        notifications[HTTPCodes.OK];
          ...
          const notify = function (el: any, error: any) {
            ...
            if (!notifications[code]) notifications[code] = [];
            notifications[code].push(el.name);
          };

          const execute = function (
            name: string,
            eventName: /*EventsName*/ string,
            fnProp
          ) {
            ...
            list.forEach(function (el) {
              ...
              new Workflow(false)
                .each(els, (x, cb) => {
                  if (x.type !== 'element') return cb();
                  fnProp(x, cb);
                })
                .intermediate((el, e) => {
                  /*if(!e)
                                            ctx.invokeEvent(eventName, r, el)*/
                  if (el.type === 'element') notify(el, e);
                })
                .done((_) => finalize());
            });
          };
          ...
          r.fs.checkPrivilege(ctx, r.path, 'canWriteProperties', (e, can) => {
            if (e || !can) {
              if (e) {
                if (!ctx.setCodeFromError(e))
                  ctx.setCode(HTTPCodes.InternalServerError);
              } else if (!can) ctx.setCodeFromError(Errors.NotEnoughPrivilege);
              return callback();
            }
            ...
            r.fs.isLocked(ctx, r.path, (e, locked) => {
              if (e || locked) {
                if (e) {
                  if (!ctx.setCodeFromError(e))
                    ctx.setCode(HTTPCodes.InternalServerError);
                } else if (locked) ctx.setCode(HTTPCodes.Locked);
                return callback();
              }
              ...
              r.propertyManager((e, pm) => {
                ...
                pm.getProperties((e, props) => {
                  ...
                  const pushSetReverseAction = (el: XMLElement) => {
                    ...
                    if (prop)
                      reverse.push((cb) =>
                        pm.setProperty(el.name, prop.value, prop.attributes, cb)
                      );
                    else reverse.push((cb) => pm.removeProperty(el.name, cb));
                  };
                  ...
                  execute(
                    ...
                    (el: XMLElement, callback) => {
                      ...
                      pm.removeProperty(el.name, (e) => {
                        if (!e) pushRemoveReverseAction(el);
                        callback(e);
                      });
                    }
                  );
