...
import {
  ResourceType,
  OpenWriteStreamMode,
} from '../../manager/fileSystem/CommonTypes';
        ...
        r.type((e, type) =>
          process.nextTick(() => {
            ...
            r.openWriteStream(
              mode,
              targetSource,
              ctx.headers.contentLength,
              (e, wStream, created) => {
                if (e) {
                  if (!ctx.setCodeFromError(e))
                    ctx.setCode(
                      e === Errors.IntermediateResourceMissing ||
                        e === Errors.WrongParentTypeForCreation
                        ? HTTPCodes.Conflict
                ...
                wStream.on('finish', (e) => {
                  if (created) ctx.setCode(HTTPCodes.Created);
                  else ctx.setCode(HTTPCodes.OK);
