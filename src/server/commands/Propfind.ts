...
import { ResourceType } from '../../manager/fileSystem/CommonTypes';
import { Workflow } from '../../helper/Workflow';
import { Resource } from '../../manager/fileSystem/Resource';
...
function parseRequestBody(ctx: HTTPRequestContext, data: Buffer): PropertyRule {
  ...
  if (ctx.headers.contentLength <= 0) return allTrue;
  ...
  try {
    ...
    if (propfind.findIndex('DAV:propname') !== -1) return onlyName;

    if (propfind.findIndex('DAV:allprop') !== -1) return allTrue;
...
export default class implements HTTPMethod {
  addXMLInfo(
    ...
    data: Buffer,
    resource: Resource,
    multistatus: XMLElementBuilder,
    _callback: (e?: Error) => void
  ): void {
    const reqBody = parseRequestBody(ctx, data);

    const response = new XMLElementBuilder('D:response');
    const callback = (e?: Error) => {
      if (e === Errors.MustIgnore || e === Errors.ResourceNotFound) e = null;
      else if (!e) multistatus.add(response);
      else {
        ...
        if (errorNumber !== null) {
          ...
          response
            .ele('D:propstat')
            .ele('D:status')
            .add(`HTTP/1.1 ${errorNumber} ${http.STATUS_CODES[errorNumber]}`);
          resource.fs.getFullPath(ctx, resource.path, (e, path) => {
            if (e) return nbOut(e);

            const p = HTTPRequestContext.encodeURL(
              ctx.fullUri(path.toString())
            );
    ...
    function displayValue(values: string[] | string, fn: () => void) {
      if (
        values.constructor === String
          ? tags[values as string].value
          : (values as string[]).some((n) => tags[n].value)
      ) {
        ++nb;
        process.nextTick(fn);
      }
    ...
    displayValue('creationdate', () => {
      resource.creationDate((e, ticks) =>
        process.nextTick(() => {
          if (!e) tags.creationdate.el.add(dateISO8601(ticks));
      ...
      resource.listDeepLocks((e, locks) => {
        ...
        for (const path in locks) {
          for (const _lock of locks[path]) {
            ...
            activelock
              .ele('D:lockscope')
              .ele('D:' + lock.lockKind.scope.value.toLowerCase());
            activelock
              .ele('D:locktype')
              .ele('D:' + lock.lockKind.type.value.toLowerCase());
            activelock.ele('D:depth').add('Infinity');
            if (lock.owner) activelock.ele('D:owner').add(lock.owner);
            activelock
              .ele('D:timeout')
              .add(`Second-${lock.expirationDate - Date.now()}`);
            activelock
              .ele('D:locktoken')
              .ele('D:href', undefined, true)
              .add(lock.uuid);
            activelock
              .ele('D:lockroot')
              .ele('D:href', undefined, true)
              .add(HTTPRequestContext.encodeURL(ctx.fullUri(path)));
          }
        }
    ...
    resource.type((e, type) =>
      process.nextTick(() => {
        ...
        resource.fs.getFullPath(ctx, resource.path, (e, path) => {
          ...
          const href =
            p.lastIndexOf('/') !== p.length - 1 && type.isDirectory
              ? p + '/'
              : p;
          ...
          if (type.isFile) {
            ...
            if (tags.getcontenttype.value) {
              ++nb;
              resource.mimeType(ctx.headers.isSource, (e, mimeType) =>
                process.nextTick(() => {
                  if (!e) tags.getcontenttype.el.add(mimeType);
                  nbOut(e);
                })
              );
            }
            ...
            if (tags.getcontentlength.value) {
              ++nb;
              resource.size(ctx.headers.isSource, (e, size) =>
                process.nextTick(() => {
                  if (!e)
                    tags.getcontentlength.el.add(
                      size === undefined ||
                        size === null ||
                        size.constructor !== Number
                        ? 0
                        : size
                    );
                  nbOut(e);
    ...
    displayValue('displayname', () => {
      ...
      if (resource.displayName) methodDisplayName = resource.displayName;

      methodDisplayName.bind(resource)((e, name) =>
        process.nextTick(() => {
          if (!e) tags.displayname.el.add(name || '');
          nbOut(e);
        })
      );
    });
    ...
    displayValue('supportedlock', () => {
      resource.availableLocks((e, lockKinds) =>
        process.nextTick(() => {
          if (e) {
            nbOut(e);
            return;
    ...
    displayValue('getetag', () => {
      resource.etag((e, etag) =>
        process.nextTick(() => {
          if (!e && tags.getetag.value) tags.getetag.el.add(etag);
          nbOut(e);
        })
      );
    });
          ...
          for (const name in properties) {
            if (reqBody.mustDisplay(name)) {
              ...
              if (reqBody.mustDisplayValue(name)) {
                ...
                else tag.attributes = property.attributes;
  ...
  unchunked(ctx: HTTPRequestContext, data: Buffer, callback: () => void): void {
    if (
      ctx.server.options.maxRequestDepth < ctx.headers.depth ||
      (ctx.headers.depth < 0 &&
        ctx.server.options.maxRequestDepth !== Infinity &&
        ctx.server.options.maxRequestDepth >= 0)
    ) {
        ...
        resource.type((e, type) =>
          process.nextTick(() => {
            ...
            const injectResourcePropfind = (
              ...
              resource.readDir(true, (e, children) =>
                process.nextTick(() => {
                  function err(e) {
                    if (!ctx.setCodeFromError(e))
                      ctx.setCode(HTTPCodes.InternalServerError);
                  ...
                  resource.fs.getFullPath(ctx, resource.path, (e, rsPath) => {
                    new Workflow()
                      .each(children, (childName, cb) => {
                        ctx.server.getResource(
                          ctx,
                          rsPath.getChildPath(childName),
                          (e, r) => {
                            if (e) return cb(e);
                            ...
                            this.addXMLInfo(ctx, data, r, multistatus, (e) => {
                              ...
                              if (depth !== 0) {
                                r.type((e, type) => {
                                  if (e || !type.isDirectory) return cb(e);
