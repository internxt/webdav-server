  ...
  _transform(chunk: string | Buffer, encoding: string, callback: Function) {
    if (this.nb < this.min) {
      ...
      if (this.nb > this.min) {
        ...
        chunk = chunk.slice(
          start,
          this.nb > this.max ? this.max - this.min + 1 + start : undefined
        );
        callback(null, chunk);
...
export function parseRangeHeader(
  ...
  const len =
    ranges.reduce((previous, mm) => mm.max - mm.min + 1 + previous, 0) +
    (ranges.length <= 1
      ? 0
      : ranges.reduce(
          ...
          r.type((e, type) => {
            ...
            r.size(targetSource, (e, size) =>
              process.nextTick(() => {
                ...
                r.mimeType(targetSource, (e, mimeType) =>
                  process.nextTick(() => {
                    if (e) {
                      if (!ctx.setCodeFromError(e))
                        ctx.setCode(HTTPCodes.InternalServerError);
                      ...
                      if (range) {
                        try {
                          const {
                            ...
                            createMultipart,
                            endMultipart,
                          } = parseRangeHeader(mimeType, size, range);
                          ...
                          if (ranges.length <= 1) {
                            ...
                            ctx.response.setHeader(
                              'Content-Range',
                              `bytes ${ranges[0].min}-${ranges[0].max}/*`
                            );
                            rstream.on('end', callback);
                            return rstream
                              .pipe(
                                new RangedStream(ranges[0].min, ranges[0].max)
                              )
                              .pipe(ctx.response);
                          }
                          ...
                          const evalNext = () => {
                            ...
                            if (sr) {
                              if (current > 0) ctx.response.write('\r\n');
