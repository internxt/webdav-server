...
import { ResourceType, SimpleCallback, Resource } from '../../manager';

import { extractOneToken } from '../../helper/IfParser';
import { LockScope, LockKind, LockType, Lock } from '../../resource';
...
function createResponse(ctx: HTTPRequestContext, lock: Lock) {
  ...
  activelock
    ...
    .add(HTTPRequestContext.encodeURL(ctx.fullUri()));
  activelock
    .ele('D:depth')
    .add(lock.depth === -1 ? 'infinity' : lock.depth.toString());
...
function createLock(ctx: HTTPRequestContext, data: Buffer, callback) {
  try {
    ...
    const scope = new LockScope(
      root.find('DAV:lockscope').elements[0].name.substr(4).toLowerCase()
    );
    const type = new LockType(
      root.find('DAV:locktype').elements[0].name.substr(4).toLowerCase()
    );
    const ownerElementIndex = root.findIndex('DAV:owner');
    const owner =
      ownerElementIndex !== -1
        ? root.elements[ownerElementIndex].elements
        : null;

    const lock = new Lock(
      new LockKind(scope, type, ctx.server.options.lockTimeout),
      ctx.user ? ctx.user.uid : undefined,
      owner,
      ctx.headers.depth === undefined ? -1 : ctx.headers.depth
    );
    ...
    const go = (r: Resource, callback: SimpleCallback) => {
      ...
      r.listDeepLocks((e, locks) => {
        ...
        if (Object.keys(locks).length > 0) {
          if (LockScope.Exclusive.isSame(type)) return callback(Errors.Locked);

          for (const path in locks)
            if (
              locks[path].some((l) =>
                LockScope.Exclusive.isSame(l.lockKind.scope)
              )
            )
    ...
    callback = (e) => {
      ...
      } else ctx.writeBody(createResponse(ctx, lock));
      _callback();
    };
