...
export function executeRequest(
  ...
  if (!method) method = this.unknownMethod;

  HTTPRequestContext.create(this, req, res, rootPath, (e, base) => {
    if (e) {
      if (
        e === Errors.AuenticationPropertyMissing ||
        e === Errors.MissingAuthorisationHeader ||
        e === Errors.BadAuthentication ||
        e === Errors.WrongHeaderFormat
      )
        base.setCode(HTTPCodes.Unauthorized);
      else base.setCode(HTTPCodes.InternalServerError);
      res.end();
      return;
    }
    ...
    if (!method.chunked) {
      ...
      if (base.headers.contentLength <= 0) {
        ...
        req.on('data', (chunk) => {
          ...
          if (index >= base.headers.contentLength) go(data);
        });
      }
    } else {
      this.invokeBeforeRequest(base, () => {
...
export function start(
  ...
  if (port && typeof port === 'number') _port = port;
  else if (callback && typeof callback === 'number') _port = callback;

  if (port && typeof port === 'function') _callback = port;
  else if (callback && typeof callback === 'function') _callback = callback;

  if (!this.server) {
    const serverCreator = this.options.https
      ? (c) => https.createServer(this.options.https, c)
      : (c) => http.createServer(c);
  ...
  this.server.listen(_port, this.options.hostname, () => {
    if (_callback) _callback(this.server);
  });
}
