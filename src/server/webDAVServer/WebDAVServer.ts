...
import { HTTPAuthentication, PrivilegeManager } from '../../user';
...
import { promisifyCall } from '../../helper/promise';
...
export class WebDAVServer {
  ...
  constructor(options?: WebDAVServerOptions) {
    ...
    for (const k in commands)
      if (k === 'NotImplemented') this.onUnknownMethod(new commands[k]());
      else this.method(k, new commands[k]());
  }

  protected isSameFileSystem(
    fs: FileSystem,
    path: string,
    checkByReference: boolean
  ): boolean {
    return (
      (checkByReference && this.fileSystems[path] === fs) ||
      (!checkByReference &&
        this.fileSystems[path].serializer().uid() === fs.serializer().uid())
    );
  }
  ...
  createExternalContext(
    callback: (error: Error, ctx: ExternalRequestContext) => void
  ): ExternalRequestContext;
  /**
   ...
   */
  createExternalContext(
    options: RequestContextExternalOptions
  ): ExternalRequestContext;
  /**
   * Create an external context with specified options.
   *
  ...
  createExternalContext(
    _options?:
      | RequestContextExternalOptions
      | ((error: Error, ctx: ExternalRequestContext) => void),
    _callback?: (error: Error, ctx: ExternalRequestContext) => void
  ): ExternalRequestContext {
  ...
  getResource(
    ctx: RequestContext,
    path: Path | string,
    callback: ReturnCallback<Resource>
  ): void {
    path = new Path(path);
  ...
  setFileSystemAsync(
    ...
    return promisifyCall((cb) =>
      this.setFileSystem(path, fs, override, (successed) =>
        cb(undefined, successed)
      )
    );
  }
  ...
  setFileSystem(
    path: Path | string,
    fs: FileSystem,
    callback: (successed?: boolean) => void
  ): void;
  /**
   * Map/mount a file system to a path.
   *
  ...
  setFileSystem(
    path: Path | string,
    fs: FileSystem,
    _override: boolean | ((successed?: boolean) => void),
    _callback?: (successed?: boolean) => void
  ): void {
    const override = _callback ? (_override as boolean) : undefined;
    const callback = _callback
      ? _callback
      : (_override as (successed?: boolean) => void);
  ...
  setFileSystemSync(
    ...
    if (!override && this.fileSystems[sPath]) return false;
  ...
  removeFileSystem(
    path: Path | string,
    callback: (nbRemoved?: number) => void
  ): void;
  /**
  ...
  removeFileSystem(
    fs_path: Path | string | FileSystem,
    _checkByReference: boolean | ((nbRemoved?: number) => void),
    _callback?: (nbRemoved?: number) => void
  ): void {
    const checkByReference = _callback ? (_checkByReference as boolean) : true;
    const callback = _callback
      ? _callback
      : (_checkByReference as (nbRemoved?: number) => void);
  ...
  removeFileSystemSync(
    fs_path: Path | string | FileSystem,
    checkByReference: boolean = true
  ): number {
    let nb = 0;
  ...
  getFileSystemPath(
    fs: FileSystem,
    _checkByReference: boolean | ((path: Path) => void),
    _callback?: (path: Path) => void
  ): void {
    const checkByReference = _callback
      ? (_checkByReference as boolean)
      : undefined;
    const callback = _callback
      ? _callback
      : (_checkByReference as (path: Path) => void);
  ...
  getFileSystemPathSync(fs: FileSystem, checkByReference?: boolean): Path {
    checkByReference =
      checkByReference === null || checkByReference === undefined
        ? true
        : checkByReference;
  ...
  getChildFileSystems(
    parentPath: Path,
    callback: (fss: { fs: FileSystem; path: Path }[]) => void
  ): void {
  ...
  getChildFileSystemsSync(parentPath: Path): { fs: FileSystem; path: Path }[] {
    ...
    for (const fsPath in this.fileSystems) {
      ...
      if (
        pfsPath.paths.length === parentPath.paths.length + 1 &&
        startsWith(fsPath, seekPath)
      ) {
        results.push({
  ...
  getFileSystemAsync(
    path: Path
  ): Promise<{ fs: FileSystem; rootPath: Path; subPath: Path }> {
    return promisifyCall((cb) =>
      this.getFileSystem(path, (fs, rootPath, subPath) =>
        cb(undefined, { fs, rootPath, subPath })
      )
    );
  }
  ...
  getFileSystem(
    path: Path,
    callback: (fs: FileSystem, rootPath: Path, subPath: Path) => void
  ): void {
  ...
  getFileSystemSync(path: Path): {
    ...
    for (const fsPath in this.fileSystems) {
      ...
      if (path.paths.length < pfsPath.paths.length) continue;
      ...
      if (value === path.paths.length) break; // Found the best value possible.
    }

    const subPath = path.clone();
    for (const _ of best.rootPath.paths) subPath.removeRoot();
  ...
  listResourcesAsync(root?: string | Path): Promise<string[]> {
    return promisifyCall((cb) =>
      this.listResources(root, (paths) => cb(undefined, paths))
    );
  }
  ...
  listResources(
    ...
    this.getResource(this.createExternalContext(), root, (e, resource) => {
      resource.readDir(true, (e, files) => {
        if (e || files.length === 0) return callback(output);
  ...
  startAsync(port?: number): Promise<http.Server> {
    return promisifyCall((cb) =>
      this.start(port, (server) => cb(undefined, server))
    );
  }
  ...
  start(
    port?: number | WebDAVServerStartCallback,
    callback?: WebDAVServerStartCallback
  ) {
  ...
  autoSave(options?: IAutoSave) {
    ...
    else if (this.options.autoSave) fn(this.options.autoSave);
  }
