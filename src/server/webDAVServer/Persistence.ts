import {
  ...
  unserialize,
  SerializedData,
} from '../../manager/fileSystem/Serialization';
import { PhysicalSerializerVersions } from '../../manager/instances/PhysicalFileSystem';
import { VirtualSerializerVersions } from '../../manager/instances/VirtualFileSystem';
import { IAutoSave, IAutoLoad } from '../WebDAVServerOptions';
import { HTTPRequestContext } from '../RequestContext';
import { SimpleCallback } from '../../manager/fileSystem/CommonTypes';
import { Readable } from 'stream';
import * as zlib from 'zlib';
import * as fs from 'fs';

function defaultSerializers() {
  return VirtualSerializerVersions.instances.concat(
    PhysicalSerializerVersions.instances
  );
}
...
export function load(
  ...
  const fSerializers = serializers
    ? serializers.concat(defaultSerializers())
    : defaultSerializers();

  unserialize(data, fSerializers, (e, udata) => {
    if (!e) this.fileSystems = udata;
    callback(e);
  });
}
...
export function autoLoad(callback: SimpleCallback) {
  ...
  if (!options.treeFilePath) {
    if (!this.options.autoSave || !this.options.autoSave.treeFilePath)
      return callback(
        new Error('The "treeFilePath" of the "autoLoad" option is not found.')
      );
    else options.treeFilePath = this.options.autoSave.treeFilePath;
  }
...
function loadDefaultIAutoSaveParam(options: IAutoSave) {
  if (!options.streamProvider) options.streamProvider = (cb) => cb();
  if (!options.onSaveError) options.onSaveError = () => {};
...
export class AutoSavePool {
  constructor(
    options: IAutoSave,
    saveFn: (callback: (error: Error, data: any) => void) => void
  ) {
    ...
    this.saveFn((e, data) => {
      if (e) {
        ...
        this.options.streamProvider((inputStream, outputStream) => {
          if (!inputStream) inputStream = zlib.createGzip();
          if (!outputStream) outputStream = inputStream;
          const fileStream = fs.createWriteStream(
            this.options.tempTreeFilePath
          );
          outputStream.pipe(fileStream);

          inputStream.write(JSON.stringify(data));
          inputStream.end((e) => {
            if (e) {
              this.options.onSaveError(e);
              this.saveIfNext();
          ...
          fileStream.on('close', () => {
            fs.unlink(this.options.treeFilePath, (e) => {
              if (e && e.code !== 'ENOENT') {
                // An error other than ENOENT (no file/folder found)
                this.options.onSaveError(e);
                this.saveIfNext();
                return;
              }
              ...
              fs.rename(
                this.options.tempTreeFilePath,
                this.options.treeFilePath,
                (e) => {
                  if (e) this.options.onSaveError(e);
                  this.saveIfNext();
                }
              );
            });
